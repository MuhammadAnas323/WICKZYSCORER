...
class _SquadSetupScreenState extends ConsumerState<SquadSetupScreen> {
  ...
  Future<void> _editPlayer(ScorerPlayer player) async {
    ...
    if (!mounted) return;
    setState(() {
      final isTeam1 = player.teamId == _match?.team1Id;
  ...
  Future<void> _continueToToss() async {
    final match = _match;
    if (match == null) return;
    if (!_canStart) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Select at least one player for each team'),
            ...
            child: ListView(
              ...
              children: [
                _teamSection(
                  ...
                  accent: AppColors.pitchGreenLight,
                ),
                const Gap(24),
                _teamSection(
                ...
                ElevatedButton.icon(
                  ...
                  label: Text(_saving ? 'Saving…' : 'Start Scoring',
                      style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                  onPressed: (_saving || !_canStart) ? null : _continueToToss,
                ),
                if (!_canStart) ...[
                  const Gap(8),
                  const Text(
                    'Select at least one player for each team to start scoring.',
                ...
                TextButton(
                  onPressed: _saving
                      ? null
                      : () => context.pushReplacement('/scorer/toss?matchId=${match.id}'),
                  child: const Text('Continue to Toss', style: TextStyle(color: Colors.white54)),
                ),
                const Gap(40),
              ],
            ),
      ...
      children: [
        Row(
          children: [
            ...
            Text('${xi.length} in squad',
                style: TextStyle(color: accent, fontSize: 12, fontWeight: FontWeight.bold)),
          ],
        ),
        const Gap(4),
        const Text(
          'Tap the checkbox to include a player in the playing XI.',
        ...
        if (players.isEmpty)
          Container(
            ...
            child: Column(
              children: [
                const Text('No players yet — add the squad members for this team.',
                ...
                OutlinedButton.icon(
                  ...
                  onPressed: () => _addPlayer(teamId, teamId == _match?.team1Id ? _team1 : _team2),
                ),
              ],
            ),
          )
  ...
  Widget _playerTile({
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: inXi ? accent.withOpacity(0.08) : AppColors.darkSurface,
        borderRadius: BorderRadius.circular(12),
        border: Border.all(color: inXi ? accent.withOpacity(0.4) : Colors.white10),
      ),
      child: Row(
        children: [
          Expanded(
            child: InkWell(
              ...
              child: Row(
                children: [
                  Icon(
                    inXi ? Icons.check_circle_rounded : Icons.radio_button_unchecked,
